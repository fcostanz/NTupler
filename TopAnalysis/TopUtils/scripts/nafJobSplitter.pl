...
sub peek {
    ...
    if ($self->queue() =~ /\@(.+)/) {
        ...
        my $jid = $self->fullId();
        system("qrsh -l h_rt=00:01:00 -l h=$1 -l h_vmem=100M -now n 'sh -c \"cd \$TMP; cd ../$jid.*/njs_*/; tail -n 200 -f stdout.txt\"'");
    } else {
...
sub logIntoHost {
    ...
    if ($self->queue() =~ /\@(.+)/) {
        print "Please wait, this can take up to a few minutes...\n";
        my $jid = $self->fullId();
        system("qrsh -l h_rt=00:05:00 -l h=$1 -l h_vmem=100M -now n ");
    } else {
        die "Didn't find hostname\n";
    }
}
